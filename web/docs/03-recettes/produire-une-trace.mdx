...
S'il y a des fonctions dans le code, on ajoute la pile d'appels pour indiquer
- quand on entre dans le code d'une autre fonction
- pour chaque ligne, quelles sont les fonctions qui sont actives
...
````
6. Crée toi un projet kotlin ou Android pour pouvoir valider ta trace en utilisant le débogueur.
