    ...
    On veut faire la trace d'exécution lorsqu'on ouvre l'application client, en assumant que le serveur roule et que tout se passe bien.
        ...
        <Row>
            <Column>
                ...
                interface Service {
                    @GET("plus100/{nombre}")
                    fun plus100(@Path("nombre") nombre: Int): Call<Int>
                }
                ...
                public class Controlleur {
                    ...
                    @GetMapping("/plus100/{nombre}")
                    public @ResponseBody Integer plus100(@PathVariable Integer nombre)  {
                        return nombre + 100;
                    }
                }
                ```
            ...
            <Column>
                ...
                |```retrofit.service.plus100(42).enqueue(object : retrofit2.Callback<Int> {```  |lance la requête http://10.0.2.2:8080/plus100/42 en asynchrone|onCreate|
                |```return nombre + 100;```|retourne 142|plus100|
                |```if (response.isSuccessful) {```|on vérifie si la réponse *response* reçue du serveur est un succès|onResponse|
                |```Toast.makeText(this@MainActivity, response.body()!!.toString(), Toast.LENGTH_LONG).show()```|on affiche "142" dans un Toast|onResponse|
