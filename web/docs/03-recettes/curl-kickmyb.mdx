...
```bash
curl.exe -X POST "https://kickmyb-server.onrender.com/id/inscription"   -H "Content-Type: application/json"  -d '{ "nom": "test@example.com", "motDePasse": "123456",  "confirmationMotDePasse": "123456" }'   -c cookies.txt
```
...
```bash
curl.exe -X POST "https://kickmyb-server.onrender.com/id/connexion" -H "Content-Type: application/json" -d '{ "nom": "test@example.com", "motDePasse": "123456" }' -c cookies.txt
```
...
```bash
curl.exe "https://kickmyb-server.onrender.com/tache/accueil" -b cookies.txt
```
...
```bash
curl.exe -X POST "https://kickmyb-server.onrender.com/tache/ajout" -H "Content-Type: application/json" -d '{ "nom": "Finir le TP2", "dateLimite": "2026-03-15T23:59:00" }' -b cookies.txt
```
...
```bash
curl.exe "https://kickmyb-server.onrender.com/tache/detail/1" -b cookies.txt -v
```
...
```bash
curl.exe "https://kickmyb-server.onrender.com/tache/progres/1/50" -b cookies.txt -v
```
...
```bash
curl.exe -X POST "https://kickmyb-server.onrender.com/id/deconnexion" -b cookies.txt -v
```
...
```bash
curl.exe "https://kickmyb-server.onrender.com/tache/accueil" -b cookies.txt -o reponse.json
```
