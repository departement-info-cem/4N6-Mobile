...
(qui indique une solution quand c'est possible). 
Cela inclut les cas où l'utilisateur donne de mauvaises données (validation, trop court, existant etc.), 
les erreurs d'accès au serveur etc. Cet aspect sera corrigé en détail sur:
...
Vous implantez les messages d'erreur pour tous les autres écrans de l'application.
